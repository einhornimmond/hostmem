 ...
 * hostmem_memory_block_free(&tx_vec_back(&v)->signature, alloc);
 * tx_vec_pop(&v);
 *
 * // removing all of them: the same duty, once per element
 * record *tx;
 * HOSTMEM_BVEC_FOREACH(tx_vec, &v, tx, i) { hostmem_memory_block_free(&tx->signature, alloc); }
 * tx_vec_free(&v);
 * @endcode
 *
 ...
 * HOSTMEM_BVEC_STATIC(tx_vec, record, 7)  // 128 elements per bucket
 *
 * tx_vec v;
 * tx_vec_init(&v, NULL);                 // NULL => malloc/free
 * record *slot;
 * if (tx_vec_emplace(&v, &slot) == HOSTMEM_SUCCESS) {
 *   // fill *slot in place, no copy
 * }
 * tx_vec_free(&v);
 *
 * // shared across translation units, split the generation:
 * HOSTMEM_BVEC_DECLARE(tx_vec, record, 7, extern)  // header
 * HOSTMEM_BVEC_DEFINE(tx_vec, record, 7,)          // exactly one .c file
 * @endcode
 *
 ...
 * assert in HOSTMEM_BVEC_DECLARE() admits.
 */
#define HOSTMEM_BVEC_BUCKET_BYTES(type, log2_bucket) ((uint32_t)(sizeof(type) << (log2_bucket)))
...
/**
 * @brief Release a block obtained from hostmem_bvec_raw_alloc().
 *
 ...
 *
 * The counterpart of hostmem_bvec_index_grow(): both take slot counts, so the conversion to
 ...
 */
#define HOSTMEM_BVEC_DECLARE(name, type, log2_bucket, scope)                                       \
                                                                                                   \
  static_assert(                                                                                   \
  ...
  typedef struct name {                                                                            \
    ...
    hostmem *allocator;       /**< Allocator, or NULL for malloc/free. */                          \
                                                                                                   ...
                                                                                                   \
  scope hostmem_result name##_init(name *v, hostmem *allocator);                                   \
  scope hostmem_result name##_reserve(name *v, uint32_t element_count);                            \
  scope hostmem_result name##_shrink(name *v);                                                     \
  scope void name##_clear(name *v);                                                                \
  scope void name##_free(name *v);                                                                 \
  scope hostmem_result name##_grow(name *v, type **out_slot);                                      \
  scope hostmem_result name##_emplace(name *v, type **out_slot);                                   \
  scope hostmem_result name##_push(name *v, type value);                                           \
  scope hostmem_result name##_push_ptr(name *v, const type *value);                                \
  scope hostmem_result name##_pop(name *v);                                                        \
  ...
  scope hostmem_result name##_copy_to(const name *v, type *dst, uint32_t dst_capacity);
...
#define HOSTMEM_BVEC_DEFINE(name, type, log2_bucket, scope)                                        \
  ...
  scope hostmem_result name##_init(name *v, hostmem *allocator) {                                  \
    if (!v) return HOSTMEM_ERROR_NULL_POINTER;                                                     \
    v->buckets = NULL;                                                                             \
    v->bucket_count = 0;                                                                           \
    v->bucket_capacity = 0;                                                                        \
    v->tail = NULL;                                                                                \
  ...
  scope hostmem_result name##_reserve(name *v, uint32_t element_count) {                           \
    ...
    if (needed <= v->bucket_count) return HOSTMEM_SUCCESS;                                         \
    ...
    while (v->bucket_count < needed) {                                                             \
      type *bucket = (type *)hostmem_bvec_raw_alloc(                                               \
          HOSTMEM_BVEC_BUCKET_BYTES(type, log2_bucket), v->allocator                               \
      );                                                                                           \
      if (!bucket) return HOSTMEM_ERROR_OUT_OF_MEMORY;                                             \
  ...
  scope hostmem_result name##_shrink(name *v) {                                                    \
    ...
    if (!i) {                                                                                      \
      ...
      /* of stranding it                                                                 */        \
      if (hostmem_bvec_index_free((void **)v->buckets, v->bucket_capacity, v->allocator)) {        \
        v->buckets = NULL;                                                                         \
        v->bucket_capacity = 0;                                                                    \
      }                                                                                            \
      return HOSTMEM_SUCCESS;                                                                      \
    }                                                                                              \
    if (i == v->bucket_capacity) return HOSTMEM_SUCCESS;                                           \
    /* a refused tightening costs only unused pointer slots, so it is not an error */              \
    if (hostmem_bvec_index_grow((void ***)&v->buckets, v->bucket_capacity, i, v->allocator)) {     \
      v->bucket_capacity = i;                                                                      \
    }                                                                                              \
    return HOSTMEM_SUCCESS;                                                                        \
  }                                                                                                \
  ...
  scope void name##_free(name *v) {                                                                \
    ...
    for (i = v->bucket_count; i > 0; --i) {                                                        \
      hostmem_bvec_raw_free(v->buckets[i - 1], bucket_bytes, v->allocator);                        \
    }                                                                                              \
    hostmem_bvec_index_free((void **)v->buckets, v->bucket_capacity, v->allocator);                \
  ...
  scope hostmem_result name##_grow(name *v, type **out_slot) {                                     \
    if (!v || !out_slot) return HOSTMEM_ERROR_NULL_POINTER;                                        \
    ...
    if (next >= v->bucket_count) {                                                                 \
      ...
      bucket = (type *)hostmem_bvec_raw_alloc(                                                     \
          HOSTMEM_BVEC_BUCKET_BYTES(type, log2_bucket), v->allocator                               \
      );                                                                                           \
      if (!bucket) return HOSTMEM_ERROR_OUT_OF_MEMORY;                                             \
      v->buckets[v->bucket_count++] = bucket;                                                      \
  ...
  scope hostmem_result name##_pop(name *v) {                                                       \
    if (!v->size) return HOSTMEM_ERROR_ARRAY_INDEX_OUT_OF_BOUNDS;                                  \
    v->size--;                                                                                     \
    if (--v->tail_used == 0) {                                                                     \
      if (v->size) {                                                                               \
        v->tail_index--;                                                                           \
  ...
  scope hostmem_result name##_copy_to(const name *v, type *dst, uint32_t dst_capacity) {           \
    ...
    if (!v || !dst) return HOSTMEM_ERROR_NULL_POINTER;                                             \
    if (dst_capacity < v->size) return HOSTMEM_ERROR_DESTINATION_BUFFER_TO_SMALL;                  \
 ...
 *
 * Convenience shorthand for HOSTMEM_BVEC_DECLARE() + HOSTMEM_BVEC_DEFINE() with `static inline`
...
#define HOSTMEM_BVEC_STATIC(name, type, log2_bucket)                                               \
  ...
  HOSTMEM_BVEC_DEFINE(name, type, log2_bucket, static inline)
 ...
 */
#define HOSTMEM_BVEC_FOREACH(name, vec, item_ptr, index_var)                                       \
